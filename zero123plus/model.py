...
class MVDiffusion(pl.LightningModule):
    ...
    def forward_vision_encoder(self, images):
        ...
        # Get global embeddings from the vision encoder
        global_embeds = self.pipeline.vision_encoder(image_pt, output_hidden_states=False).image_embeds
        global_embeds = global_embeds.unsqueeze(-2)

        # Updated encode_prompt method with num_images_per_prompt
        prompt_embeds = self.pipeline.encode_prompt(
            prompt="",  # Adjust as necessary
            num_images_per_prompt=1,  # Typically, 1 unless you want more embeddings
            device=self.device,
            do_classifier_free_guidance=False
        )[0]

        # Combine embeddings with ramping coefficients
        ramp = global_embeds.new_tensor(self.pipeline.config.ramping_coefficients).unsqueeze(-1)
        encoder_hidden_states = prompt_embeds + global_embeds * ramp
